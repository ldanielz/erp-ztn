...
async function register(req, res) {
  ...
  try {
    ...
    await logEvent('info', `New user registered: ${email}`, user.id)
...
async function login(req, res) {
  ...
  try {
    ...
    if (!user) {
      await logEvent('warning', `Failed login attempt for email: ${email}`)
      return res.status(401).json({ message: 'Invalid credentials' })
    }
    const ok = await bcrypt.compare(password, user.password_hash)
    if (!ok) {
      await logEvent('warning', `Failed login attempt for user: ${email}`, user.id)
      return res.status(401).json({ message: 'Invalid credentials' })
    }

    await logEvent('info', `User logged in: ${email}`, user.id)
...
async function updateProfile(req, res) {
  try {
    ...
    if (currentPassword && newPassword) {
      ...
      if (!passwordMatch) {
        await logEvent('warning', `Failed profile update (password mismatch) for user: ${user.email}`, userId)
    ...
    await logEvent('info', `User updated profile: ${updatedUser.email}`, userId)
